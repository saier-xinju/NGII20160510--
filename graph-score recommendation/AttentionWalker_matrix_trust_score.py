...
def Init_graph2(options,retention):
    print "Initing the attention graph G2 with retention................."
    G2 = [[0 for i in range(NUM_USERS)] for i in range(NUM_USERS)]
    G3 = [[0 for i in range(NUM_USERS + NUM_ITEMS)] for i in range(NUM_USERS + NUM_ITEMS)]
    ...
    for line in r.readlines():
        ...
        fan = info[0]
        fan = int(fan[1:])
        nrofRetention = int((len(info)-1) * retention)
        print nrofRetention
        for i in range(1,nrofRetention+1):
            t = info[i]
            info2 = t.split(":")
            attention = int(info2[0])
            G2[fan-1][attention-1] = 1
    r.close()
